...
func newServer(id uint64, tr hbbft.Transport, nodes []uint64, port int) *Server {
...
func makeNetwork(n int) []*Server {
	...
	for i := 0; i < n; i++ {
		...
		fmt.Printf("Port:%d\n", (port + i))
		nodes[i] = newServer(uint64(i), transports[i], makeids(n), (port + i))
	}
	connectTransports(transports)
	return nodes
}

func connectTransports(tp []hbbft.Transport) {
	for i := 0; i < len(tp); i++ {
		for ii := 0; ii < len(tp); ii++ {
			if ii == i {
				continue
			}
			tp[i].Connect(tp[ii].Addr(), tp[ii])
		}
	}
}
